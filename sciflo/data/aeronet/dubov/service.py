...
from sciflo.data.aeronet.subset import parse, subsetOneSite, DATA_VARS_EXTRA
...
def geoRegionQuery(datasetName, level, version,
    ...
    if level == "2.0" or level == "2":
        metaTable="meta"
        dataTable="data"
    else:
        raise RuntimeError, "extra aeronet data unavailable for level %s" % (level)
    ...
    for x in rows:
        ...
        if responseGroups == 'Small':
            ...
            dataRows = subsetOneSite(dburi, metaTable, dataTable, DATA_VARS_EXTRA, fname, startDatetime, endDatetime, latMin, latMax, lonMin, lonMax)
            if responseGroups == 'Large':
                xmlText += writeAsXML(DATA_VARS_EXTRA,dataRows)
                ...
                if len(dataRows) == 0:
                    ...
                    xmlText += "    <urls>\n"
                    path = os.path.join(outDir,RANDOM_TAG+"."+level+".extra."+fname)
                    path = os.path.abspath(writeToFileAsCSV(DATA_VARS_EXTRA,dataRows,path))
