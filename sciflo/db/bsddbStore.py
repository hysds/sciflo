...
class BsddbStore(Store):
    ...
    def __init__(self, name, fieldsList, dbHome, dbName):
        ...
        #get db
        self._dbHandle = dbtablesCDB.bsdTableDB(self._dbName, dbhome=self._dbHome,create=createFlag) #, dbflags=db.DB_INIT_CDB)

        #create table with columns if it doesn't exist
        if self._name not in self._dbHandle.ListTables():
            try: self._dbHandle.CreateTable(self._name,self._fieldsList)
            except dbtablesCDB.TableAlreadyExists: pass
    
    def __del__(self): self.close()
    
    def close(self):
        """Close db then dbenv to release all locks."""
        try: self._dbHandle.db.close()
        except: pass
        try: self._dbHandle.env.close()
        except: pass
    ...
    def _remove(self, id):
        ...
        self._dbHandle.Delete(self._name,conditions={ idField: lambda x: pickle.loads(x)==id })
    ...
    def _query(self, queryField, queryValue, returnFieldsList):
        ...
        for result in resultSet:
            ...
            for field in returnFieldsList:
                if result[field] is None: val = None
                else: val = pickle.loads(result[field])
