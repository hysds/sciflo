...
class Store(object):
    ...
    def add(self, *args, **kargs):
        ...
        fieldDataList = []
        for f in range(len(self._fieldsList)):
            fieldDataList.append(None)
    ...
    def queryMultipleFields(self, queryDict, returnField=None, getFieldsListFlag=None):
        ...
        # return columns
        if returnField is None:
            returnFieldsList = list(queryDict.keys())
        else:
            returnFieldsList = getListFromUnknownObject(returnField)
    ...
    def queryUnique(self, queryField, queryValue, returnField=None):
        ...
        # query
        (resultSet, returnedFields) = self.query(
            queryField, queryValue, returnField, getFieldsListFlag=1)

        # check if unique
        if len(resultSet) == 1:
            if len(returnedFields) == 1:
