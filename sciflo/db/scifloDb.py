...
def getIndexedFields(xml, recordTag):
    ...
    if root.tag == recordTag: recElt = root
    else:
        try: recElt = root.xpath('.//_default:%s' % recordTag,namespaces=nsDict)[0]
        except: recElt = root.xpath('.//%s' % recordTag)[0]
    retList = []
...
def getSqlCreateInfo(xml, recordTag):
    ...
    retList = []
    for subElt in recElt: retList.append((subElt.tag, subElt.get('sqltype','text')))
    return retList
...
def getCreateSql(tableName, xml, recordTag, autoKey=False):
    ...
    if autoKey: retStr += "    id int primary key auto_increment,\n"
    fieldStrList = ["    %s %s" % (field, sqltype) for field, sqltype in sqlCreateInfoList]
    return retStr + ",\n".join(fieldStrList) + ")"
...
def insertXml(location, tableName, xml, recordTag='record', keyTags=[],
    ...
    sqlhub.processConnection = connection
    connection.debug = debug
    if iterateMode: xmlRecList = xmlList2PyLoX(xml, recordTag)
    else: xmlRecList = [xml]
...
class ScifloDbTable(object):
    ...
    def __init__(self, location, xmlSchema):
        ...
        for fieldElt in self.fieldElts:
            ...
            #get nullable flag
            nullable = None
            if null is None: pass
            elif re.search(r'yes', null, re.IGNORECASE): nullable = False
            else: pass

            #get key spec
            alternateID = None
            unique = None
            if key == '' or re.search(r'mul', key, re.IGNORECASE): pass
            elif re.search(r'pri', key, re.IGNORECASE):
                alternateID = True; nullable = None; self.keyCol = id
            ...
            if typ.startswith('char'):
                ...
                if matchChar: colLen = matchChar.group(1)
            ...
            colArgsList = []
            if alternateID == True: colArgsList.append("alternateID=True")
            if nullable: colArgsList.append("notNone=True")
            if unique == True: colArgsList.append("unique=True")
            if colLen is not None: colArgsList.append("length=%s" % colLen)
            if defaultVal is not None: colArgsList.append("default='%s'" % defaultVal)
    ...
    def update(self, xml):
        ...
        kwargs = {}
        for thisElt in elt:
            if thisElt.text == key: continue
            else:
                if thisElt.tag == 'localtime': fieldName = 'ltime'
                else: fieldName = thisElt.tag
                typ = self.fieldInfoDict[fieldName]['type']
                if typ in ('float','double') or \
                    typ.startswith('double'): kwargs[fieldName] = float(thisElt.text)
                elif typ == 'datetime': kwargs[fieldName] = getDatetimeFromString(thisElt.text)
                else: kwargs[fieldName] = thisElt.text
        try:
    ...
    def query(self,id):
        ...
        try: res = eval("self.table.%s(id)" % self.keyColSelectMethodStr)
    ...
    def remove(self,id):
        ...
        try: eval("self.table.%s(id).destroySelf()" % self.keyColSelectMethodStr)
        except SQLObjectNotFound: return False
        except: raise
        return True
    ...
    def removeAll(self):
        ...
        self.connection.query(self.connection.sqlrepr(Delete(self.table.sqlmeta.table,None)))
        return True
