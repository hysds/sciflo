...
class FetchNotAllowedError(Exception): pass
class InstallNotAllowedError(Exception): pass
...
def inIpRange(host, spec):
    ...
    for i in range(0,4):
        ...
        if re.search(r'^\d+$', specOcts[i]):
            if hostOcts[i] == int(specOcts[i]): continue
            else: return False
...
def fetchFromAllowed(netloc):
    ...
    for iprange in re.split(r'\s+', scp.getParameter('allowCodeFetchFrom')):
        if iprange == '': continue
        if inIpRange(sciflo.utils.getHostIp(netloc), iprange): return True
    return False
