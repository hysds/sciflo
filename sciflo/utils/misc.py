...
def getThreadSafeRandomObject(jumpahead=19594):
    ...
    rndm = Random(); rndm.jumpahead(jumpahead)
    return rndm
...
def getTempfileName(suffix=None, dir=None):
    ...
    if suffix: tmpBase += suffix
    if dir: return mktemp(tmpBase, UID, dir)
    else: return mktemp(tmpBase, UID)
...
def extractZipfile(file, dir=".", verbose=False):
    ...
    for dirs in dirlist:
        ...
        for dir in dirs:
            ...
            if dir and not os.path.isdir(dirname): os.mkdir(dirname)
            prefix = dirname
    # extract files
    for fn in filelist:
        try:
            fnDir = os.path.dirname(fn)
            if fnDir not in ['', '.'] and not os.path.isdir(fnDir): os.makedirs(fnDir)
            out = open(fn, 'wb')
...
def isBundle(bundleFile, returnCmd=False):
    ...
    if tarfile.is_tarfile(bundleFile):
        ...
        if returnCmd: return extractZipfile
        else: return True
    else: return False
...
in your sciflo configuration." % loc
                ...
                if not os.path.isdir(md5dir): os.makedirs(md5dir)
...
in your sciflo configuration." % loc
            ...
            if os.path.isdir(item): shutil.copytree(item,dest)
            else: shutil.copy(item,dest)
        if unpackBundles: unpackBundle(dest, dir=destDir, loc=loc)
...
class UrlBaseTracker(object):
    ...
    def __init__(self,rootDir,urlBase):
        ...
        #set urlBase
        if urlBase.endswith('/'): self._urlBase = re.sub(r'/$','',urlBase)
        else: self._urlBase = urlBase
    ...
    def getUrl(self, path, returnFileUrl=False):
        ...
        if not absPath.startswith(self._rootDir):
            if returnFileUrl: return 'file://%s%s' % (socket.getfqdn(), absPath)
            else: return absPath
...
def convertImage(inputFile, outputFile, format=None, convert=None,
    ...
    if forcePortrait and os.path.splitext(inputFile)[1] == '.ps':
        ...
        fileContents = f.read()
        f.close()
        match = re.search(r'Orientation:\s*landscape',fileContents,re.IGNORECASE)
        if match: rotate = -90
    try:
        im = Image.open(inputFile)
        if rotate: im = im.rotate(rotate)
        ...
        #try using convert
        if rotate:
            cmd = "convert -rotate %s -crop 0x0 -size 1280x1024 %s %s" % (abs(rotate), inputFile, outputFile)
        else:
            cmd = "convert -crop 0x0 -size 1280x1024 %s %s" % (inputFile, outputFile)
        runCommandLine(cmd)
    return True
...
def convertToPng(inputFile, outputFile=None):
    ...
    if outputFile is None: outputFile = os.path.splitext(inputFile)[0]+'.png'
...
def convertToJpeg(inputFile, outputFile=None):
    ...
    if outputFile is None: outputFile = os.path.splitext(inputFile)[0]+'.jpg'
    convertImage(inputFile,outputFile)
    return outputFile
...
def getFlashMovie(imageFiles,outputFile=None):
    ...
    if outputFile is None: outputFile = os.path.basename(getTempfileName(suffix='.swf'))
    pngFiles = []
    for imageFile in imageFiles:
        if imageFile.endswith('.png'): pngFile = imageFile
        else: pngFile = convertToPng(imageFile)
        pngFiles.append(pngFile)
    movieFile = outputFile + '.tmp1.swf'
    output = runCommandLine('png2swf -o %s -X 700 -Y 500 -j 100 %s' % (movieFile, ' '.join(pngFiles)))
    ...
    output = runCommandLine('swfcombine -o %s -X 700 -Y 500 %s loader=%s movie=%s' % \
                            (outputFile, os.path.join(sys.prefix,'share','swftools','swfs',
                             ...
                             os.path.join(sys.prefix,'share','swftools','swfs','default_loader.swf'),
                             movieFile2))
    return outputFile
...
def linkFile(src, dest):
    ...
    if os.path.isdir(dest): dest = os.path.abspath(os.path.join(dest,os.path.basename(src)))
...
<myConfig xmlns="${sflNs}"
    ...
    <conversionOperators>${defConvElts}
        ...
        f.write(t.substitute(sflNs=SCIFLO_NAMESPACE,xsdNs=XSD_NAMESPACE, pyNs=PY_NAMESPACE,
                             sflWorkDir=os.path.join(gettempdir(),'scifloWork-%s' % userName),
                             sflExecDir=os.path.join(userScifloDir,'exec'), cachePort='8001',
                             hostCert=os.path.join(sys.prefix, 'ssl', 'hostcert.pem'),
                             hostKey=os.path.join(sys.prefix, 'ssl', 'hostkey.pem'),
...
def getUserScifloConfig(userConfigFile=None, globalConfigFile=None):
    ...
    if userConfigFile is None: userConfigFile = userConfig
    ...
    gridProtocol = ucfElt.xpath('.//_default:sflProtocol',namespaces=ucfNs)[0].text
    gridPort = ucfElt.xpath('.//_default:sflPort',namespaces=ucfNs)[0].text
    gridProxyUrlNodes = ucfElt.xpath('.//_default:sflProxyUrl',namespaces=ucfNs)
    if len(gridProxyUrlNodes) == 0: gridProxyUrl = ''
    else: gridProxyUrl = gridProxyUrlNodes[0].text
    baseUrl = ucfElt.xpath('.//_default:baseUrl',namespaces=ucfNs)[0].text
    if baseUrl is None: baseUrl = ''
    exposerProtocol = ucfElt.xpath('.//_default:exposerProtocol',namespaces=ucfNs)[0].text
    exposerPort = ucfElt.xpath('.//_default:exposerPort',namespaces=ucfNs)[0].text
    exposerProxyUrlNodes = ucfElt.xpath('.//_default:exposerProxyUrl',namespaces=ucfNs)
    if len(exposerProxyUrlNodes) == 0: exposerProxyUrl = ''
    else: exposerProxyUrl = exposerProxyUrlNodes[0].text
    dbPort = ucfElt.xpath('.//_default:dbPort',namespaces=ucfNs)[0].text
    dbUser = ucfElt.xpath('.//_default:dbUser',namespaces=ucfNs)[0].text
    dbPassword = ucfElt.xpath('.//_default:dbPassword',namespaces=ucfNs)[0].text
    allowCodeFetchFrom = ucfElt.xpath('.//_default:allowCodeFetchFrom',namespaces=ucfNs)[0].text
    allowCodeInstallFrom = ucfElt.xpath('.//_default:allowCodeInstallFrom',namespaces=ucfNs)[0].text
    htmlBaseHrefNodes = ucfElt.xpath('.//_default:htmlBaseHref',namespaces=ucfNs)
    if len(htmlBaseHrefNodes) == 0: htmlBaseHref = ''
    else: htmlBaseHref = htmlBaseHrefNodes[0].text
    cgiBaseHrefNodes = ucfElt.xpath('.//_default:cgiBaseHref',namespaces=ucfNs)
    if len(cgiBaseHrefNodes) == 0: cgiBaseHref = ''
    else: cgiBaseHref = cgiBaseHrefNodes[0].text
    gmapKeyNodes = ucfElt.xpath('.//_default:gmapKey',namespaces=ucfNs)
    if len(gmapKeyNodes) == 0: gmapKey = ''
    else: gmapKey = gmapKeyNodes[0].text
    timeout = ucfElt.xpath('.//_default:workUnitTimeout',namespaces=ucfNs)[0].text
    if timeout is None: timeout = 86400
    ...
    if globalConfigFile is None:
        ...
        gcfElt.xpath('.//_default:workUnitRootWorkDir',namespaces=gcfNs)[0].text = workDir
        gcfElt.xpath('.//_default:workUnitStoreType',namespaces=gcfNs)[0].text = workUnitStoreType
        gcfElt.xpath('.//_default:scheduleStoreType',namespaces=gcfNs)[0].text = scheduleStoreType
        gcfElt.xpath('.//_default:workUnitStoreHome',namespaces=gcfNs)[0].text = workUnitStoreHome
        gcfElt.xpath('.//_default:scheduleStoreHome',namespaces=gcfNs)[0].text = scheduleStoreHome
        gcfElt.xpath('.//_default:workUnitTimeout',namespaces=gcfNs)[0].text = timeout
        gcfElt.xpath('.//_default:cacheHome',namespaces=gcfNs)[0].text = cacheHome
        gcfElt.xpath('.//_default:cachePort',namespaces=gcfNs)[0].text = cachePort
        gcfElt.xpath('.//_default:hostCert',namespaces=gcfNs)[0].text = hostCert
        gcfElt.xpath('.//_default:hostKey',namespaces=gcfNs)[0].text = hostKey
        gcfElt.xpath('.//_default:gridProtocol',namespaces=gcfNs)[0].text = gridProtocol
        gcfElt.xpath('.//_default:gridPort',namespaces=gcfNs)[0].text = gridPort
        gcfElt.xpath('.//_default:gridProxyUrl',namespaces=gcfNs)[0].text = gridProxyUrl
        gcfElt.xpath('.//_default:baseUrl',namespaces=gcfNs)[0].text = baseUrl
        gcfElt.xpath('.//_default:exposerProtocol',namespaces=gcfNs)[0].text = exposerProtocol
        gcfElt.xpath('.//_default:exposerPort',namespaces=gcfNs)[0].text = exposerPort
        gcfElt.xpath('.//_default:exposerProxyUrl',namespaces=gcfNs)[0].text = exposerProxyUrl
        gcfElt.xpath('.//_default:dbPort',namespaces=gcfNs)[0].text = dbPort
        gcfElt.xpath('.//_default:dbUser',namespaces=gcfNs)[0].text = dbUser
        gcfElt.xpath('.//_default:dbPassword',namespaces=gcfNs)[0].text = dbPassword
        gcfElt.xpath('.//_default:allowCodeFetchFrom',namespaces=gcfNs)[0].text = allowCodeFetchFrom
        gcfElt.xpath('.//_default:allowCodeInstallFrom',namespaces=gcfNs)[0].text = allowCodeInstallFrom
        gcfElt.xpath('.//_default:htmlBaseHref',namespaces=gcfNs)[0].text = htmlBaseHref
        gcfElt.xpath('.//_default:cgiBaseHref',namespaces=gcfNs)[0].text = cgiBaseHref
...
def writePickleFile(obj, outputFile=None):
    ...
    if outputFile is None: outputFile = os.path.abspath(os.path.basename(getTempfileName(suffix='.pkl')))
...
def writeTextFile(obj, outputFile=None):
    ...
    #if output file not specified, get random name
    if outputFile is None: outputFile = os.path.abspath(os.path.basename(getTempfileName(suffix='.txt')))
    f = open(outputFile,'w')
    f.write(str(obj))
    f.close()
    return outputFile
...
def findFile(filename, rootDir):
    ...
    for root,dirs,files in os.walk(rootDir, followlinks=True):
        for file in files:
            if file == filename: return os.path.join(root,file)
    return None
...
def findFileRegEx(expr, rootDir):
    ...
    for root,dirs,files in os.walk(rootDir, followlinks=True):
        for file in files:
            if re.search(r'%s' % expr, file): return os.path.join(root,file)
    return None
...
def makeUrlLocal(url, noDODSFlag, dir=None):
    ...
    if noDODSFlag and isDODS(url): return None
    elif isDODS(url): return url
    else: return getFile(url, dir)
...
def makeLocal(urlArg, noDODSFlag=False, dir=None):
    ...
    if len(urlsElts) > 1:
        ...
        url = makeUrlLocal(urlsElts[0].xpath('_default:url',locsNs)[0].text,noDODSFlag, dir)
...
def runDot(dot, outputFile=None, outputType=None):
    ...
    try: dotFile, headers = urlllib.urlretrieve(dot)
    except:
        if re.search('}\s*$', dot):
            tmpFlag = True
...
def getRelativeUrl(source, target):
    ...
    if su[0] != tu[0] or su[1] != tu[1]: return target
    su = re.split("/", su[2])
    tu = re.split("/", tu[2])
    su.reverse()
    tu.reverse()
    ...
    if len(su) == 0 and len(tu) == 0: tu.append(lastPop)
