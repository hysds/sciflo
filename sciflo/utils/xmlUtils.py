...
def postCall(url, data, headers, verbose=False):
    ...
    try:
        if isXml(data):
            ...
            if not data.startswith('<?xml'): data = '<?xml version="1.0" encoding="UTF-8"?>' + data
...
def simpleList2Xml(LL,rootElement = "Rows", rowElement = "row",rootAttribsDict=None,
    ...
    if schemaUrl is not None: rootAttribsDict['schemaLocation'] = schemaUrl
    if xsiNamespace is not None: rootAttribsDict['xsi'] = xsiNamespace
...
def list2Xml(LL,headingsTuple = (), rootElement = "Rows", rowElement = "row",
    ...
    for rec in LL:
        ...
        if len(rec) != numHeadings: continue
        ...
        for x in range(numHeadings):
            ...
            else: addChildTextNodeToParentNode(recElem,heading,rec[x])
    #return getPrettyPrintXmlFromDom(xmlDoc)
    return lxml.etree.tostring(rootElem, pretty_print=True)
...
def xml2List(xmlString):
    ...
    for node in firstChild.childNodes:
        if node.firstChild.nodeType == 3: tagNames.append(node.nodeName)
...
is not 'objectid' tag: %''' % subresult[0].tag)
        ...
        for subresult in subresults[1:]:
            ...
            if type == 'xs:int': value = int(value)
            elif type == 'xs:float': value = float(value)
            elif type == 'xs:boolean': value = bool(value)
            else: pass
...
def getEltsAsList(eltsList):
    ...
    for elt in eltsList:
        ...
        if value is None or re.search(r'\s*',str(value)):
            subElts = elt.findall('*')
            if len(subElts) > 0: value = getEltsAsList(subElts)
...
def xmlList2PyLoD(xml, recordTag):
    ...
    if root.tag == recordTag: recElts = [root]
    else:
        try: recElts = root.xpath('.//_default:%s' % recordTag,namespaces=nsDict)
        except: recElts = root.xpath('.//%s' % recordTag)
    retList = []
    for recElt in recElts:
        recDict = {}
        for subElt in recElt: recDict[subElt.tag] = subElt.text
...
def xmlList2PyLoX(xml, recordTag):
    ...
    retList = []
    for recElt in recElts: retList.append(lxml.etree.tostring(recElt,
                                                              pretty_print=True))
    return retList
...
def validateXml(inputXml, schemaXml):
    ...
    try:
        if os.path.isfile(inputXml): f = open(inputXml)
        else: f = StringIO(inputXml)
        if os.path.isfile(schemaXml): s = open(schemaXml)
...
class ScifloConfigParser(object):
    ...
    def __init__(self, file=None):
        ...
        if file: self._configFile = file
        else: self._configFile = getUserScifloConfig()
    ...
    def getParameter(self,param):
        ...
        if result in [None, 'None', ''] or result.text in [None, 'None', '']: return None
        else: return str(result.text)
    ...
    def getParameterViaXPath(self,xpath):
        ...
        if result in [None, 'None', '']: return None
        else: return str(result.text)
...
def parseElement(elt, returnChildren=False):
    ...
    kids = None
    if elt.tag is None:
        if returnChildren: return (None, None, None, None, kids)
    ...
    if 'from' in elt.attrib: value = elt.get('from', None)
        ...
        if len(eltKids) > 0:
            ...
            if len(kids) == 1:
                ...
                value = '\n'.join([lxml.etree.tostring(i, pretty_print=True).strip() for i in kids])
                value = value.strip()
        else:
            value = elt.text
            if value is not None: value = value.strip()
    if returnChildren: return (namespace, tag, typ, value, kids)
...
def getTypedValue(typ, val):
    ...
    #return typed val
    if typ in ('base64Binary', 'date', 'dateTime', 'ISODateTime', 'time'): return val
    else: return coerceFunction(val)
...
def transformXml(xmlFileOrString,xslFileOrString):
    ...
    #get xml doc
    if os.path.isfile(xmlFileOrString): doc = libxml2.parseFile(xmlFileOrString)
    else: doc = libxml2.parseDoc(xmlFileOrString)

    #get xsl doc
    if os.path.isfile(xslFileOrString): styledoc = libxml2.parseFile(xslFileOrString)
    else: styledoc = libxml2.parseDoc(xslFileOrString)
