...
def publicizeResultFiles(result, urlBaseTrackerObj):
    ...
    if isinstance(result, bytes) or isinstance(result, str):
        ...
        newResult = []
        for r1 in result:
            newResult.append(publicizeResultFiles(r1, urlBaseTrackerObj))
...
class WorkUnitExecutionHandler(object):
    ...
    def __init__(self, workUnitOwner, workUnitType, workUnitCall, workUnitArgs, wuStoreConfig,
        ...
        self._postExecutionHandlers = getListFromUnknownObject(
        ...
        if self._wuType == 'sciflo':
            if isinstance(self._wuArgs[0], dict) and \
                ...
                localExecutionMode = self._wuArgs[0].get(
                    'localExecutionMode', False)
        ...
        if self._publicBaseUrl:
            ...
            self._urlBaseTrackerObj = UrlBaseTracker(
                self._wuRootDir, self._publicBaseUrl)
        ...
        for postExecHdl in self._postExecutionHandlers:
            if postExecHdl is None:
                postExecIds.append(None)
            else:
                postExecIds.append(postExecHdl.getIdString())
        ...
        if wuClass is None:
            raise WorkUnitExecutionHandlerError(
                "Unimplemented WorkUnit subclass for type %s." % self._wuType)
    ...
    def executeWorkUnit(self, timeout=86400):
        ...
        try:
            copyToDir(self._stageFiles, self._workDir, unpackBundles=True)
        ...
        if self._publicBaseUrl:
            self._result = publicizeResultFiles(
                self._result, self._urlBaseTrackerObj)
        else:
        ...
        try:
            testStr = pickle.dumps(self._result, -1)
        except Exception as e:
            self._result = WorkUnitExecutionHandlerError(
                "Unpickleable result: %s" % self._result)
    ...
    def runPostExecutionHandlers(self):
        ...
        for postExecutionHandler in self._postExecutionHandlers:
            ...
            # get post result
            postResult = postExecutionHandler.execute(
                self._result, self._workDir)

            # is result a local file and are we publicizing?  If so publicize.
            if self._publicBaseUrl:
                postResult = publicizeResultFiles(
                    postResult, self._urlBaseTrackerObj)
...
class WorkUnitManager(object):
    ...
    def __init__(self, owner, type, call, args, storeConfig, workRootDir, stageFiles=None,
        ...
        if self._postExecutionConfigList:
            ...
            for outputIdx, conversionFuncStr in self._postExecutionConfigList:
                self._postExecutionHandlers.append(
                    PostExecutionHandler(outputIdx, conversionFuncStr))
        else:
            self._postExecutionHandlers = None

        # create callback
        if self._callbackConfig:
            self._callback = getCallback(self._callbackConfig)
        else:
            self._callback = None
    ...
    def run(self):
        res = sciflo.grid.funcs.workUnitExecutionHandlerWorker(
            self._executor, self._timeout)
        ...
        if status in finishedStatusList:
            if self._callback:
                self._callback(self._wuid, status)
    ...
    def killSelf(self, exceptionObj=WorkUnitKilled("Work unit was killed.")):
        ...
        # do callback
        if self._callback:
            self._callback(self._wuid, cancelledStatus)
...
def cancelWorkUnit(wuid, configFile=None, remove=False):
    ...
    if remove:
        ...
        if wuid.startswith('workunitconfigid-'):
            workUnitScheduleConfig = getScheduleConfigFromConfiguration(
                configFile)
            ...
            storeHandler = WorkUnitStoreHandler(storeConfig)
            try:
                ret = storeHandler.removeWorkUnit(wuid)
            except WorkUnitStoreHandlerError as e:
                if re.search(r'is not already in the store', str(e)):
                    pass
                else:
                    raise
        return True
        ...
        try:
            ...
            if status == retryStatus and isinstance(curStatus, str):
                if curStatus.startswith('retry_'):
                    retryNum = int(
                        re.search(r'^retry_(\d+)$', curStatus).group(1))
                    if retryNum <= 5:
                        scheduleHandler.setStatusByWuid(
                            wuid, retryStatus + '_%d' % (retryNum + 1))
                        return
                else:
...
def nonforkingManager(owner, type, call, args, storeConfig, workRootDir, stageFiles=None,
    ...
    if postExecutionConfigList:
        ...
        for outputIdx, conversionFuncStr in postExecutionConfigList:
            postExecutionHandlers.append(
                PostExecutionHandler(outputIdx, conversionFuncStr))
    else:
        postExecutionHandlers = None
