...
class WorkUnitConfig(object):
    ...
    def __init__(self, procCount, id, typ, call, args, stageFiles=[],
        ...
        self._procCount = procCount; self._id = id; self._type = typ
    ...
    def addPostExecutionType(self, outputIndex, postExecutionType):
        ...
        try:   #check if conversion already exists
            ...
            self._postExecutionTypeList.append((outputIndex, postExecutionType))
...
class UnresolvedArgument(object):
    ...
    def getPostExecutionOutputIndex(self): return self._postExecutionOutputIndex
...
class Sciflo(object):
    ...
    def __init__(self, xmlDoc, globalInputArgs=[], globalInputDict={},debugMode=False):
        ...
        self._flowProcessesProcess = doc.findall(ns('sf:flow/sf:processes/sf:process'))
        ...
        if len(self._globalInputDict) > 0: self._inputDict = self._globalInputDict
        else: self._inputDict = {}
        self.globalInputs = []
        inputIndex = 0
        for globalInputElt in self._flowInputs:
            ...
            try: self._inputDict[eltTag] = self._globalInputArgs[inputIndex]
            except: pass
            inputIndex += 1
    ...
    def _resolveInlineBinding(self, bindingElt):
        ...
        #parallel map python
            ...
            if job_queue is None:
                raise ScifloError("You must specify 'job_queue' attribute for binding type 'map'.")
        ...
        #parallel python
            ...
            async = bindingElt.get('async', 'false').lower()
            if job_queue is None:
                raise ScifloError("You must specify 'job_queue' attribute for binding type 'parallel'.")
            call = '%s|%s|%s' % (val, job_queue, async)
            return (wuType, endpoint, call)
        #handle python function, soap, binary, script, xquery, and bindings
        else:
            if typ in ('binary','script'): wuType = 'executable'
            elif typ == 'python': wuType = 'python function'
            else: wuType = typ
...
This module may be a staged file or bundle: %s''' % str(e)))
    ...
    def _resolveFromGlobalInputs(self, tag, val):
        ...
        if val == '@#inputs':
            ...
            if matchingElt is None: matchingElt = self._flowInputs.find(tag)
        else:
            match = re.search(r'^@#inputs\.(\w+)$', val)
            if match:
                matchingElt = self._flowInputs.find(NS['sf']+match.group(1))
                if matchingElt is None: matchingElt = self._flowInputs.find(match.group(1))
            else:
    ...
    def _resolveFromProcessId(self, tag, val, resolvingProcId):
                ...
                if match2:
                    ...
                    for index, outputElt in enumerate(outputElts):
                        ...
                        if outputTag == inputName:
                            if numOutputs == 1: outputTagIndex = None
        ...
        if xpathStr:
            if xpathStr.startswith('xpath:'): xpathStr = 'sf:%s' % xpathStr
            if not xpathStr.startswith('sf:xpath:'): xpathStr = 'sf:xpath:%s' % xpathStr
        if xpathStr: eltType = xpathStr
    ...
    def resolve(self):
        ...
        def _resolveInputs(wuArgs, previousProcId, processCount, inputsType, inputsElt, root=True):
            ...
            for inputElt in inputsList:
                ...
                #skip if comment
                if isinstance(inputElt,lxml.etree._Comment): continue

                #get input info
                (inputNs,inputTag,inputType,inputVal, eltKids) = parseElement(inputElt, returnChildren=True)
                ...
                #implicit?
                if inputVal is None:
                    if inputsType == 'arglist': inputVal = '@#inputs.%s' % inputTag
                    else: inputVal = ''
                ...
                if inputVal.startswith('@#inputs.') or \
                   ...
                   inputVal == '@#inputs':
                    ...
                    for child in inputElt.getchildren(): newElt.append(child)
                        ...
                        #no resolving needed
                            ...
                            if inputsType == 'arglist':
                                ...
                                if inputVal is None:
                                    ...
                                    #prompt for input
                                    (resolvedInputType,resolvedInputArg) = self._promptForInput()
                                ...
                                if resolvedInputType is not None:
                                    ...
                                    resolvedInputArg = convFunc(resolvedInputArg)
                            #do document
                            else:
                                wuArgs.append(None)
                                continue
                ...
                if isinstance(resolvedInputType, str):
                    ...
                    if xpathMatch:
                        ...
                        xpathDigest = getHexDigest(['xpath',xpathMatchStr,[resolvedInputArg],[]])
                        foundXpathWu=None
                        ...
                        if foundXpathWu: resolvedInputArg = UnresolvedArgument(foundXpathWu)
                        else:
                            ...
                            self._workUnitConfigs.append(implicitXpathWorkUnitConfig)
                            self._workUnitConfigsForDot.append(copy.deepcopy(implicitXpathWorkUnitConfig))
                            self._implicitWorkUnitConfigs.append(implicitXpathWorkUnitConfig)
                ...
                if inputType is None or inputType == resolvedInputType: pass
                else:
                    ...
                    if resolvedFrom in ('global input','redirect') or \
                        ...
                        #get xpath
                        xpathMatch = re.search(r'^(?:sf:)?(?:xpath:)(.*)$', inputType, re.IGNORECASE)
                        if xpathMatch:
                            implicitType = 'xpath'
                        ...
                        if found: resolvedInputArg = UnresolvedArgument(found)
                        #otherwise add an implicit work unit
                            ...
                            implicitWorkUnitConfig = WorkUnitConfig(processCount, wuId,
                                implicitType, convFuncStr, [resolvedInputArg],
                                [])
                            implicitWorkUnitConfig.setImplicitFlag(True)

                            #append to list of work units and implicit work units
                            self._workUnitConfigs.append(implicitWorkUnitConfig)
                            self._workUnitConfigsForDot.append(copy.deepcopy(implicitWorkUnitConfig))
                            self._implicitWorkUnitConfigs.append(implicitWorkUnitConfig)
                    ...
                    #type to that process' postExecution list.
                        ...
                        for wuConfig in self._workUnitConfigs:
                            ...
                            if wuConfig.getId() == resolvingProcId:
                                ...
                                resolvedInputArg.setPostExecutionOutputIndex(postExecIndex)
                ...
                else: wuArgs.append(resolvedInputArg)

                inputEltIdx += 1

            #if document inputs type
            if inputsType == 'document':
                wuArgs = DocumentArgsList(lxml.etree.tostring(inputsElt,pretty_print=True), wuArgs)
        ...
        for proc in procs:
            ...
            wuType, wuCallEndpoint, wuCall = self._resolveInlineBinding(inlineBindingElt)

            stageFiles = []
            wuArgs = []
            # Add module or binary files if type is python function or executable
            if wuType == 'python function' or wuType == 'executable':
                if wuCallEndpoint: stageFiles.append(wuCallEndpoint)
            elif wuType in ('soap','post'): wuArgs.append(wuCallEndpoint)
        ...
        for flowOutput in self._flowOutputs:
            #skip if comment
            if isinstance(flowOutput,lxml.etree._Comment): continue
            ...
            if isinstance(resolvedType,str):
                ...
                if xpathMatch:
                    xpathMatchStr = xpathMatch.group(1)
                    xpathDigest = getHexDigest(['xpath',xpathMatchStr,[resolvedArg],[]])
                    foundXpathWu = None
                    for implicitWuConfig in self._implicitWorkUnitConfigs:
                        if xpathDigest == implicitWuConfig.getHexDigest():
                            foundXpathWu = implicitWuConfig.getId()
                    if foundXpathWu: resolvedArg = UnresolvedArgument(foundXpathWu)
                    else:
            ...
            if eltType is None or eltType == resolvedType:
                ...
                if globalOutputVal: resolvedArg.addRewrite(globalOutputVal)
                            ...
                            if extMatch:
                                ...
                                for flowProc in self._flowProcessesProcess:
                                    if str(flowProc.get('id')) == resolvingProcId:
                                        procOutputs = flowProc.find(ns('sf:outputs'))
                                        idx = resolvedArg.getOutputIndex()
                                        if idx is None: idx = 0
                                        procOutputFile = procOutputs.getchildren()[idx].text
                                        if procOutputFile is None: break
                                        origBase = os.path.splitext(os.path.basename(str(procOutputFile)))[0]
                                        resolvedArg.addRewrite('.'.join([origBase,thisExt]))
                                        break
                        break
    ...
    def getDot(self):
        ...
        if self.dot is not None: return self.dot
        ...
        for wuConfig in self._workUnitConfigsForDot:
            ...
            if wuConfig.getImplicitFlag(): typ = 'implicit'
            else: typ = 'explicit'
            deps = []
            args = wuConfig.getArgs()
            for arg in args:
                if isinstance(arg,UnresolvedArgument): deps.append(processId2IdxMap[arg.getId()])
        ...
        for flowOutput in self._flowOutputs:
            ...
            if eltNs is None and eltTag is None and eltType is None and eltVal is None:
                continue
            outputConfigList.append((eltTag,processId2IdxMap[self._flowOutputConfigs[outputIdx].getId()]))
            outputIdx += 1
        self.dot = dotFlowChartFromDependencies(processConfigList, outputConfigList)
        return self.dot
    ...
    def getFullDot(self):
        ...
        #return if already got dot
        if self.fullDot is not None: return self.fullDot
        ...
        for globalInput in self.globalInputs:
            lxml.etree.SubElement(globalInputsElt,'input').set('id',globalInput)
        ...
        for wuConfig in self._workUnitConfigsForDot:
            ...
            if wuConfig.getImplicitFlag():
                ...
                for origInElt in origInElts:
                    if isinstance(origInElt,lxml.etree._Comment): continue
                ...
                for origOutElt in origOutElts:
                    if isinstance(origOutElt,lxml.etree._Comment): continue
            ...
            if mode in ('soap','post'): args = args[1:]
            if mode in ('rest','template', 'cmdline'): args = args[1::2]
            ...
            for i in range(len(args)):
                if isinstance(args[i],UnresolvedArgument):
                    ...
                    if resolvingProcOutputIndex is None: resolvingProcOutputIndex = '0'
                    inputsElt[i].set('from','process')
                    inputsElt[i].set('val','%s:%s' % (resolvingProcId,resolvingProcOutputIndex))
                else:
                    if i in wuConfig.argIdxsResolvedGloballyDict:
                        inputsElt[i].set('from','global')
                        inputsElt[i].set('val',wuConfig.argIdxsResolvedGloballyDict[i])
                    else:
        ...
        for flowOutput in self._flowOutputs:
            ...
            if resProcOutputIdx is None: resProcOutputIdx = '0'
    ...
    def _annotateSvg(self,svgStr):
        ...
        for pre in self._namespacePrefixDict:
            if pre == '_default': sflNsDict[None] = self._namespacePrefixDict[pre]
            else: sflNsDict[pre] = self._namespacePrefixDict[pre]
        if None not in sflNsDict: sflNsDict[None] = self._namespacePrefixDict['sf']
        ...
        for inputElt in self._flowInputs: sflInputsElt.append(inputElt)
    ...
    def getSvg(self):
        ...
        if self.svg is not None: return self.svg
        self.svg = self._annotateSvg(runDot(self.getDot(), None)) #None forces svg
        return self.svg
    ...
    def getFullSvg(self):
        ...
        #return if already set
        if self.fullSvg is not None: return self.fullSvg
        self.fullSvg = self._annotateSvg(runDot(self.getFullDot(), None)) #None forces svg
        return self.fullSvg
    ...
    def writeGraph(self,outputFile):
        ...
        else: runDot(self.getDot(), outputFile)
    ...
    def writeFullGraph(self,outputFile):
        ...
        else: runDot(self.getFullDot(), outputFile)
