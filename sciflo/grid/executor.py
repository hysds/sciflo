import os, time, gc, logging, types, lxml, threading, hashlib, urllib, copy
...
SCIFLO_PUBLICIZE_FIELDS = ['jsonFile', 'workDir', 'outputDir', 'svgFile', 'executionLog']
...
def scifloInfo(info=None, **kargs):
    ...
    if info is None:
        info = {}
        for f in SCIFLO_INFO_FIELDS: info[f] = kargs.get(f, None)
    else:
        for f in kargs:
            if f in SCIFLO_INFO_FIELDS: info[f] = kargs[f]
    return info
...
class ScifloExecutorError(Exception): pass
...
class ScifloExecutor(object):
    ...
    def __init__(self, sflString, args={}, workers=4, workerTimeout=None,
        ...
        elif isinstance(self.args, (types.ListType, types.TupleType)):
            self.sciflo = Sciflo(self.sflString, self.args)
        else: raise ScifloExecutorError("Unrecognized type for args: %s" % \
                                        type(self.args))
        self.sciflo.resolve()
        self.scifloName = self.sciflo.getName()
        self.wuConfigs = self.sciflo.getWorkUnitConfigs()
        if scifloid is None: self.scifloid = generateScifloId()
        else: self.scifloid = scifloid
        ...
        if DEBUG_PROCESSING: self.logger = processing.process.getLogger()
        else: self.logger = logging.getLogger(self.scifloName)
        ...
        #sciflo work dir
        if workDir is None: workDir = self.gsc.getWorkUnitWorkDir()
        ...
        if self.cacheName is None: self.pdict = None
        else:
            try: self.pdict = PersistentDict(self.cacheName, pickleVals=True)
            except Exception, e:
...
for sciflo '%s': %s.  No cache will be used." % (self.scifloName, e),
        ...
        if self.writeGraph: self.sciflo.writeGraph(self.svgFile)
        ...
        if self.publicize: self.publicizeUbt = self.ubt
        else: self.publicizeUbt = None
...
getWorkUnit(): %s\n%s" % (str(e), getTb()))
                ...
                self.updateStatus('WorkUnit status for "%s": %s' % (procId, readyStatus), wu.getInfo())
            else:
                wuid = None
                appRes = w
                self.updateStatus('WorkUnit status for "%s": %s' % (procId, waitingStatus), wu.getInfo())
    ...
    def resolveArgs(self, wuConfig):
        ...
        for arg in argsList:
            ...
            if isinstance(arg, UnresolvedArgument):
                ...
                if isinstance(newArg, NoResult): unresolvedArgsCount += 1
                else: pass
                newArgsList.append(newArg)
            else: newArgsList.append(arg)
    ...
    def resolveArg(self, unresArg):
        ...
        try:
            ...
            if isinstance(resolvingRes, NoResult): return resolvingRes
            ...
            if rewriteFile:
                if os.sep in rewriteFile: rewriteFile = os.path.basename(rewriteFile)
                rewriteFile = os.path.join(self.outputDir, rewriteFile)
                if isUrl(resolvingRes) or os.path.exists(str(resolvingRes)):
                    try: localPath = self.ubt.getLocalPath(resolvingRes)
                    except: localPath = None
                    ...
                    if localPath:
                        ...
                        rewriteFile = os.path.join(self.outputDir, rewriteFile)
                        urllib.urlretrieve(resolvingRes, rewriteFile)
                    
                    #publicize?
                    if self.publicize:
                        resolvingRes = self.ubt.getUrl(rewriteFile)
                    else: resolvingRes = rewriteFile
                else: open(rewriteFile, 'w').write("%s\n" % str(resolvingRes))
...
sciflo '%s': %s\n%s" % (resolvingId, self.scifloName, str(e), getTb()),
    ...
    def resolveDocumentInput(self, wuConfig, docArgList):
        ...
        if wuType in ('soap','post'): argIdx = 1
        else: argIdx = 0
            ...
            if docArgList[argIdx] is not None:
                ...
                if format == 'CDATA':
                    if isXml(eltTxt):
                        ...
                        eltTxt = None; cdataBlockElt.text = None
                    else: eltTxt = addCDATAPlaceholders(eltTxt)
                else:
                    ...
                    #interpolate xml fragment
                        ...
                        if isXml(tmpXml):
                            ...
                            if len(eltChildKids) == 0: pass #no children to add
                            else:
                                for i in eltChild.getchildren(): elt.append(i)
                                eltTxt = None
                        #interpolate string
                        else: pass
                elt.text = eltTxt
                if elt.get('from', None): del elt.attrib['from']
                if format: del elt.attrib['format']
            argIdx += 1
        docList = [replaceCDATAPlaceholders(
            lxml.etree.tostring(i, pretty_print=True)) for i in inputsElt]
        doc = '\n'.join(docList)
        if wuType in ('soap','post'): return [docArgList[0], doc]
        else: return [doc]
    ...
    def dispatchWorker(self, wu):
        ...
        self.updateStatus('WorkUnit status for "%s": %s' % (procId, sentStatus), wu.getInfo())
...
for '%s' in sciflo '%s': %s\n%s" % (workDir, linkDir, procId, self.scifloName,
        ...
        #lookup cache?
        if self.lookupCache: cacheName = self.cacheName
        else: cacheName = None
    ...
    def spawn(self):
        ...
        for procId in self.procIds:
            ...
            if isinstance(self.applyResultsDict[procId],
                          (WorkUnitConfig, processing.pool.ApplyResult)): pass
...
'%s'." % self.scifloName, extra={'id': self.scifloid})
    ...
    def execute(self):
        ...
        #spawn starters
        try: runLockedFunction(self.lock, self.spawn)
        except Exception, e:
...
%s\n%s" % self.executionError)
                self.annDoc.addGlobalOutput(None, self.output) #write to global
                if isinstance(self.executionError[1], CancelledWorkUnit):
                    finalStatus = cancelledStatus
                else: finalStatus = exceptionStatus
                self.executionError = map(str, self.executionError) #force str
            else:
...
'%s'." % ((endTime - startTime), self.scifloName), extra={'id': self.scifloid})
            ...
            if isinstance(self.output, ScifloExecutorError): numOutputs = 1
            else: numOutputs = len(self.output)
            if numOutputs == 1:
    ...
    def callback(self, callbackResult):
        ...
        try: runLockedFunction(self.lock, self.handle, callbackResult)
        except Exception, e:
...
sciflo '%s':%s\n%s" % (self.scifloName, str(e), getTb())
            ...
            self.executionError = ('callback', ScifloExecutorError(emessage),
                                   getTb())
            try: runFuncWithRetries(5, self.event.set)
            except Exception, e:
...
in sciflo '%s': %s\n%s" % (self.scifloName, str(e), getTb())
    ...
    def handle(self, callbackResult):
        ...
        updateJson(info['jsonFile'], info, stringifyKeys=STRINGIFY_FIELDS,
                   ...
                   publicizeKeys=WORK_UNIT_PUBLICIZE_FIELDS,
                   pickleKeys=PICKLE_FIELDS)
        self.updateStatus('WorkUnit status for "%s": %s' % (procId, calledBackStatus), info)
...
with status '%s'.  Handling result." %
    ...
    def _handle(self, procId, info):
        ...
        if self.publicize and info['call'] not in ('sciflo.utils.makeLocal', 'sciflo.utils.makeLocalNoDods'):
            self.resultsDict[procId] = publicizeResultFiles(info['result'], self.ubt)
        else: self.resultsDict[procId] = info['result']
        
        #append work unit's execution log to sciflo's execution log
        f = open(info['executionLog']); wuLog = f.read(); f.close()
        self.logger.debug("Execution log for '%s': %s" % (procId, wuLog), extra={'id': self.scifloid})
        ...
        if isinstance(info['result'], Exception):
            ...
            return
        else: self.handleResult(procId, info)
...
'%s'.  Error in detecting results from resultsDict from '%s' callback." %
    ...
    def handleResult(self, procId, info):
        ...
        #get our own pdict
        if self.cacheName is None: pdict = None
        else:
            try: pdict = PersistentDict(self.cacheName, pickleVals=True)
            except Exception, e:
                self.logger.debug("Got exception trying to get PersistentDict \
for handleResult() for procId '%s' in sciflo '%s': %s.  No cache will be used."\
        ...
        if info['workerStatus'] == cachedStatus: logHead = "Cached result"
        else: logHead = "Result"
        logRes = str(res)
        if len(logRes) > 80: logRes = "%s..." % logRes[0:79]
        ...
        updateJson(info['jsonFile'], info, stringifyKeys=STRINGIFY_FIELDS,
                   ...
                   publicizeKeys=WORK_UNIT_PUBLICIZE_FIELDS,
                   pickleKeys=PICKLE_FIELDS)
        self.updateStatus('WorkUnit status for "%s": %s' % (procId, postExecutionStatus), info)
        ...
        for i, (resIdx, funcStr) in enumerate(postExecList):
            ...
            if self.lookupCache and pdict is not None:
                try: pePklFile = pdict[postExecHex]
                except Exception, e:
...
'%s' in sciflo '%s': %s" % (procId, self.scifloName, (resIdx, funcStr)),
            ...
            if self.publicize:
                self.postExecResultsDict[procId][i] = \
                    publicizeResultFiles(postExecResult.val, self.ubt, dir=info['workDir'])
            else:
                self.postExecResultsDict[procId][i] = \
                    getAbsPathForResultFiles(postExecResult.val, dir=info['workDir'])
 ...
 %s" % (procId, self.scifloName, (resIdx, funcStr)),
        ...
        if self.publicize:
            ...
            updateJson(thisInfo['jsonFile'], thisInfo,
                       ...
                       publicizeKeys=WORK_UNIT_PUBLICIZE_FIELDS,
                       pickleKeys=PICKLE_FIELDS)
            self.updateStatus('WorkUnit status for "%s": %s' % (procId, thisInfo['workerStatus']), thisInfo)
        else:
            ...
            updateJson(info['jsonFile'], info, stringifyKeys=STRINGIFY_FIELDS,
                       ...
                       publicizeKeys=WORK_UNIT_PUBLICIZE_FIELDS,
                       pickleKeys=PICKLE_FIELDS)
            self.updateStatus('WorkUnit status for "%s": %s' % (procId, info['workerStatus']), info)
...
'%s' to cache under '%s' in sciflo '%s': %s\n%s" % (procId,
    ...
    def handleError(self, procId, info):
        ...
        try:
            ...
            if isinstance(res, CancelledWorkUnit): status = cancelledStatus
            else: status = exceptionStatus
            info = workUnitInfo(info, status=status)
            updateJson(info['jsonFile'], info, stringifyKeys=STRINGIFY_FIELDS,
                       ubt=self.publicizeUbt,
                       publicizeKeys=WORK_UNIT_PUBLICIZE_FIELDS,
                       pickleKeys=PICKLE_FIELDS)
            self.updateStatus('WorkUnit status for "%s": %s' % (procId, status), info)
        except Exception, e:
...
for '%s' in sciflo '%s': %s\n%s" % (procId, self.scifloName, str(e), getTb()),
        ...
        for i in range(len(self.output)):
            ...
            if isinstance(o, UnresolvedArgument):
                ...
                if procId == resolvingId:
                    ...
                    #link result
                    if isinstance(resolvingRes, (types.StringType,
                        types.UnicodeType)) and os.path.exists(resolvingRes):
    ...
    def resolveAndSpawn(self):
        ...
        for thisProcId in self.procIds:
            ...
            if not isinstance(wuConfig, WorkUnitConfig): continue
            ...
            if resolved:
                ...
                self.updateStatus('WorkUnit status for "%s": %s' % (thisProcId, readyStatus), wu.getInfo())
                self.dispatchWorker(wu)
        ...
        try:
            ...
            if isinstance(result, Exception):
                if executor is not None:
                    ...
                    if isinstance(executor.outputUrl, types.StringTypes):
                        message += '%s\n\n\n' % executor.outputUrl
                    else:
                        message += '%s\n\n\n' % executor.outputDir
                else:
            ...
            print "Got error trying to notify %s by email: %s" % (address, getTb())
            pass
