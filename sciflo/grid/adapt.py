...
class Adapt:
            ...
            def newFn(*args, **kwds):
                ...
                for i, type in enumerate(types):
                    ...
                    if conversionFn is None:
                        ...
                        # in and out types equal, so no conversion needed
                        if inType in self.synonyms(outType):
                            continue
                        else:
                            conversionFn = self.findConversion(
                                inType, outType, verbose=Verbose)
                    try:
    ...
    def unregisterConversion(self, inType, outType, conversionFn, verbose=Verbose):
        self._registry.unregister(
            inType, outType, conversionFn, verbose=verbose)
        return self
...
class MissingConversionError(AdaptationError):
    pass
...
class ConversionRegistry:
    ...
    def addTriples(self, triples, verbose=Verbose):
        ...
        for triple in triples:
            self.register(*triple)
        return self

    def register(self, inType, outType, conversionFn, verbose=Verbose):
        inType = ns(getTypeString(inType))
        outType = ns(getTypeString(outType))
        self.convert.setdefault(inType, {}).setdefault(
            outType, set()).add(conversionFn)
        if verbose:
            warn('Registered conversion %s: %s -> %s' %
    ...
    def find(self, inType, outType, verbose=Verbose):
        ...
        inType = ns(getTypeString(inType))
        outType = ns(getTypeString(outType))
        inTypes = typeSynonyms(inType)
        outTypes = typeSynonyms(outType)
        conversions = None
        for inType in inTypes:
            if inType in self.convert:
                for outType in outTypes:
                    if outType in self.convert[inType]:
                        # set of potential conversion functions
                        fns = self.convert[inType][outType]
                        if verbose:
                            warn('Found conversions %s: %s -> %s' %
                ...
                if conversions:
                    if verbose:
                        warn('Found conversion chain %s: %s -> %s' %
    ...
    def _seekChain(self, inType, outType, verbose, chain=[], typesSeen=set()):
        ...
        for ty in self.convert[inType]:
            ...
            link = (inType, ty)
            if verbose:
                warn('Trying link (%s -> %s)' % link)
            candidate = chain + [link]
            if ty in typeSynonyms(outType):
                return candidate
            else:
                candidate = self._seekChain(
                    ty, outType, verbose, candidate, typesSeen)
                if candidate:
                    return candidate
                else:
                    continue
        ...
        def convert(inObj, verbose=verbose):
            ...
            for link in conversions:    # execute each link in the conversion chain
                ...
                for fn in link:         # for each link, try each of the conversion functions
                    try:
                        if verbose:
                            warn('Converting: Trying link conversion %s' %
                                 str(fn))
                        outObj = fn(inObj)
...
def numericPart(s):
    try:
        return re.match(r'([-+0-9\.]+([eE][-+0-9]+)?)', s).group(1)
    except:
        return s
...
_DefaultRegistryTriples = \
    [['str', 'float', float],
     ...
     ['float', 'str', float2str],
     # XML function parses xmlDocString and returns an ElementTree object
     ['str', ElementTree, XML]
     ]
...
if __name__ == '__main__':
    ...
    adapt.registerConversion(
        ElementTree, 'py:ListOfDict', xml2ListOfDict_2step)
