...
workUnitManagerStoreFieldsList = [
    'wuid', 'digest', 'executePid', 'managerPid', 'entryTime']
...
class WorkUnitStoreHandler(object):
    ...
    def __init__(self, storeConfig):
        ...
        if not isinstance(storeConfig, StoreConfig):
            raise WorkUnitStoreHandlerError(
                "Must specify a StoreConfig object.")
    ...
    def addWorkUnit(self, wuid, digest=None, owner=None, status=None,
        ...
        if self.isWorkUnitIdHere(wuid):
            raise WorkUnitStoreHandlerError(
                "Work unit id %s is already in the store." % wuid)
    ...
    def removeWorkUnit(self, wuid):
        ...
        if not self.isWorkUnitIdHere(wuid):
            raise WorkUnitStoreHandlerError(
                "Work unit id %s is not already in the store." % wuid)
    ...
    def getValueByWorkUnitId(self, wuid, field):
        ...
        if self.isWorkUnitIdHere(wuid):
            ...
            qTryRes = None
            for qTry in range(2):
                qTryRes = self.queryUnique(
                    'wuid', wuid, ['digest', 'status'], dataStore=True)
            ...
            if digest is not None and re.search(r'^sciflowuid-', digest) and status == cachedStatus:
                ...
                if len(fields) == 1:
                    if fields[0] in cachedFields:
                        return self.queryUnique('wuid', digest, fields[0],
                                                dataStore=True,
                                                cachedStore=True)
                    else:
                        return self.queryUnique('wuid', wuid, fields[0], dataStore=True)
                else:
                    cFields = []
                    dFields = []
                    for f in fields:
            ...
            # otherwise return the value from this work unit
                ...
                if len(fields) == 1:
                    return self.queryUnique('wuid', wuid, fields[0],
                                            dataStore=True)
                else:
                    return self.queryUnique('wuid', wuid, fields, dataStore=True)
    ...
    def _getRecentWorkUnitIdFromPid(self, pidField, pid):
        ...
        results = self._managerStore.query(
            pidField, pid, ['wuid', 'entryTime'])
    ...
    def _modifyByWorkUnitId(self, wuid, modifyFields, modifyValues):
        ...
        for field in modifyFields:
            ...
            if field in workUnitManagerStoreFieldsList:
                managerDataDict[field] = modifyValues[index]
            index += 1
    ...
    def setExceptionMessage(self, wuid, exceptionMessage):
        ...
        self._modifyByWorkUnitId(
            wuid, ['exceptionMessage'], [exceptionMessage])
    ...
    def setPostExecutionResults(self, wuid, postExecutionResults):
        ...
        self._modifyByWorkUnitId(wuid, ['postExecutionResults'], [
    ...
    def setTracebackMessage(self, wuid, tracebackMessage):
        ...
        self._modifyByWorkUnitId(
            wuid, ['tracebackMessage'], [tracebackMessage])
    ...
    def getValueByDigest(self, digest, field):
        ...
        for wuIds in wuIdList:
            if len(wuIds) != 1:
                raise WorkUnitStoreHandlerError(
                    "Cannot handle number of wuIds found: %s" % wuIds)
...
class ScheduleStoreHandler(object):
    ...
    def setStore(self, wuConfigId, cachedStore=False):
        # create store
        if wuConfigId is None:
            return None
        args = [self._storeName,  self._storeFieldsList]
        dbHome = os.path.join(
            self._storeArgs[0], 'wuConfigIdStores', wuConfigId)
        validateDirectory(dbHome, noExceptionRaise=True)
        args.append(dbHome)
        args.extend(self._storeArgs[1:])
        if cachedStore:
    ...
    def addWorkUnitConfig(self, wuConfigId, scifloId, wuIndex, procId, executeNodeProtocol,
        ...
        if self.isWorkUnitConfigIdHere(wuConfigId):
            raise ScheduleStoreHandlerError(
                "Work unit config id %s is already in the store." % wuConfigId)
    ...
    def removeWorkUnitConfig(self, wuConfigId):
        ...
        if not self.isWorkUnitConfigIdHere(wuConfigId):
            raise ScheduleStoreHandlerError(
                "Work unit config id %s is not already in the store." % wuConfigId)
    ...
    def getValueByWorkUnitConfigId(self, wuConfigId, field):
        ...
        if self.isWorkUnitConfigIdHere(wuConfigId):
            ...
            # get digest and status
            digest, status = self.queryUnique('wuConfigId', wuConfigId, [
                                              'digest', 'status'], dataStore=True)
            ...
            if digest is not None and re.search(r'^workunitconfigid-', digest) and status == cachedStatus:
                ...
                if len(fields) == 1:
                    if fields[0] in cachedFields:
                        return self.queryUnique('wuConfigId', digest, fields[0],
                                                dataStore=True,
                                                cachedStore=True)
                    else:
                        return self.queryUnique('wuConfigId', wuConfigId, fields[0], dataStore=True)
                else:
                    cFields = []
                    dFields = []
                    for f in fields:
            ...
            # otherwise return the value from this work unit
                ...
                if len(fields) == 1:
                    return self.queryUnique('wuConfigId', wuConfigId, fields[0],
                                            dataStore=True)
                else:
                    return self.queryUnique('wuConfigId', wuConfigId, fields, dataStore=True)
    ...
    def getValueByDigest(self, digest, field):
        ...
        # get wu config id
        wuConfigIdList = self._managerStore.query(
            'digest', digest, 'wuConfigId')
        ...
        for wuConfigIds in wuConfigIdList:
            if len(wuConfigIds) != 1:
                raise ScheduleStoreHandlerError(
                    "Cannot handle number of wuConfigIds found: %s" % wuConfigIds)
            retList.append(self.getValueByWorkUnitConfigId(
                wuConfigIds[0], field))
        return retList
    ...
    def _modifyByWorkUnitConfigId(self, wuConfigId, modifyFields, modifyValues):
        ...
        for field in modifyFields:
            dataDict[field] = modifyValues[index]
            if field in scheduleManagerStoreFieldsList:
                managerDataDict[field] = modifyValues[index]
            index += 1
    ...
    def getExecuteNodeInfo(self, wuConfigId):
        ...
        # get values
        protocol = self.getValueByWorkUnitConfigId(
            wuConfigId, 'executeNodeProtocol')
        ...
        ns = self.getValueByWorkUnitConfigId(
            wuConfigId, 'executeNodeNamespace')
    ...
    def getWorkUnitConfigIdByScifloAndProcIds(self, scifloId, procId):
        ...
        for wuConfigIds in wuConfigIdList:
            if len(wuConfigIds) != 1:
                raise ScheduleStoreHandlerError(
                    "Cannot handle number of wuConfigIds found: %s" % wuConfigIds)
            thisProcId = self.getValueByWorkUnitConfigId(
                wuConfigIds[0], ['procId'])
    ...
    def setPostExecutionResults(self, wuConfigId, postExecutionResults):
        ...
        self._modifyByWorkUnitConfigId(
            wuConfigId, ['postExecutionResults'], [postExecutionResults])
    ...
    def setResolvedFlag(self, wuConfigId, resolvedFlag):
        ...
        self._modifyByWorkUnitConfigId(
            wuConfigId, ['resolvedFlag'], [resolvedFlag])
    ...
    def setExceptionMessage(self, wuConfigId, exceptionMessage):
        ...
        self._modifyByWorkUnitConfigId(
            wuConfigId, ['exceptionMessage'], [exceptionMessage])
    ...
    def setTracebackMessage(self, wuConfigId, tracebackMessage):
        ...
        self._modifyByWorkUnitConfigId(
            wuConfigId, ['tracebackMessage'], [tracebackMessage])
    ...
    def setImplicitFlag(self, wuConfigId, implicitFlag):
        ...
        self._modifyByWorkUnitConfigId(
            wuConfigId, ['implicitFlag'], [implicitFlag])
    ...
    def setExecutionLog(self, wuConfigId, executionLog):
        ...
        self._modifyByWorkUnitConfigId(
            wuConfigId, ['executionLog'], [executionLog])
    ...
    def setFinishedTime(self, wuConfigId, finishedTime):
        ...
        self._modifyByWorkUnitConfigId(
            wuConfigId, ['finishedTime'], [finishedTime])
