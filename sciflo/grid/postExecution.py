...
def parseNamespacePrefixAndTypeString(typeString):
    ...
    if typeString is None:
        nsprefix = '*'
        val = '*'
    elif ':' in typeString: (nsprefix,val) = typeString.split(':',1)
    else:
        nsprefix = None
        val = typeString
    return (nsprefix, val)
...
def getConversionFunctionString(fromType, toType, namespacePrefixDict={}):
    ...
    for op in ops:
        ...
        else: opFromNs = userConfNsToPrefixDict[namespacePrefixDict[opFromNs]]
    ...
    if fromNsPrefix is None: fromNs = convNsToPrefixDict[namespacePrefixDict['_default']]
    elif fromNsPrefix == '*': fromNs = '*'
    else: fromNs = convNsToPrefixDict[namespacePrefixDict[fromNsPrefix]]
...
class PostExecutionHandler(object):
    ...
    def execute(self, result, workDir):
        ...
        try: retVal = self._execute(result, workDir)
        finally:
    ...
    def _execute(self, result, workDir):
        ...
        if self._resultIndex is None: inputResult = result
        else: inputResult = result[self._resultIndex]
        ...
        convFunc = getFunction(self._conversionFuncStr,addToSysPath=getUserConversionFunctionsDir())
        ...
        if isinstance(convFunc, FileConversionFunction):
            if singleArgFlag: retVal = convFunc(inputResult)
            else: retVal = [convFunc(ip) for ip in inputResult]
        else: retVal = convFunc(inputResult)
