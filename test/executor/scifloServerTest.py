...
class scifloServerTestCase(unittest.TestCase):
    ...
    def testScifloServer(self):
        ...
        try:
            ...
            for s, p in (('service server', testServicePort),
                ...
                while True:
                    ...
                    try:
                        ...
                        sc.close()
                        print(
                            ("Connected to %s at localhost:%s successfully." % (s, p)))
                        break
                    except:
                    ...
                    if loopLimit >= 30:
                        raise RuntimeError(
                            "%s failed to come up at localhost:%s." % (s, p))
                    loopLimit += 1
            ...
            while True:
                try:
                    info = loadJson(jsonFile)
                except IOError as e:
                    print(("Got IOError for %s: %s" % (jsonFile, e)))
                    time.sleep(1)
                    continue
                if info['status'] in finishedStatusList:
                    break
                time.sleep(5)
            for k in info:
                print(("%s: %s" % (k, info[k])))
            ...
            # remove soap dir
            if os.path.isdir(soapDir):
                shutil.rmtree(soapDir)
