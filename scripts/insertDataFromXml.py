...
def main():
    ...
    #get opts
    try: opts,args = getopt.getopt(sys.argv[1:],"l:t:r:k:hdu", ["location=", "table=",
  ...
  Only specify one keyTag."""
    ...
    #xml doc
    doc = args[0]; f = open(doc,'r'); xml = f.read(); f.close()

    #insert
    kargs = {'autoKey': False, 'createIfNeeded': True, 'iterateMode': False, 'forceDelete': False}
    if recordTag: kargs['recordTag'] = recordTag
    if keyTags: kargs['keyTags'] = keyTags
    if update: kargs['updateOnDup'] = True
    etree.clearErrorLog()
    try:
    ...
    except sciflo.db.NoIndexedFieldsInXmlError, e:
        print "Couldn't find indexed/keyed fields.  Specify them with -k|--keyTags  option."
        usage(); sys.exit(2)
    except Exception, e:
