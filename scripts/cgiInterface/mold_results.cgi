...
def sendBundle(wuidStr):
    ...
    if SCIFLOID_RE.search(wuid):
        jsonPath = '/%s/sciflo.json' % wuid
    else:
        jsonPath = '/%s/workunit.json' % wuid
...
if __name__ == '__main__':
    ...
    # get wuid
    if not form.has_key('wuid'):
        raise RuntimeError, "No work unit id specified."
