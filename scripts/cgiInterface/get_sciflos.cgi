...
SCIFLO_DOCS_ROOT_DIR = os.path.join(
    sys.prefix, 'share', 'sciflo', 'web', 'flows')
...
def getScifloDirectoryListing(docDir):
    ...
    files = os.listdir(scifloDocsDir)
    files.sort()
    retHtml = ''
    subdirHtml = ''
    for i in files:
        ...
        if os.path.isdir(scifloFile) and os.access(scifloFile, 5) and \
                not i.startswith('.'):
            subdirFiles = os.listdir(scifloFile)
            subdirFiles.sort()
            subSfFound = False
            for j in subdirFiles:
                if j.endswith('sf.xml'):
                    subSfFound = True
            if subSfFound:
            ...
            continue
        if not scifloFile.endswith('sf.xml'):
            continue
        scifloUrl = os.path.join(scifloDocsUrl, i)
        try:
            elt, nsDict = sciflo.utils.getXmlEtree(scifloFile)
        except:
            continue
        ...
        if len(iconNodes) == 1:
            icon = iconNodes[0].text
        else:
            icon = os.path.join(REL_URL, 'portal_images/esip.gif')
        desc = elt.xpath('./sf:flow/sf:description', namespaces=nsDict)[0].text
        if desc is None or re.search(r'^\s*$', desc):
            desc = 'No description defined in sciflo.'
        # set html for sciflo
