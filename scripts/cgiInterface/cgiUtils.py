...
#link for exceptions
exceptionLnkTpl = Template('''<a href="$result" target="_blank"><font color="red">exception</font></a>''')

#generic result
genericResultLnkTpl = Template('''<a href="$result" target="_blank">$link</a>''')

#url/link result
linkResultLnkTpl = Template('[<a href="$result" target="_blank">download</a>]')

#bundle result
bundleResultLnkTpl = Template('''[<a id="$wuid-result-$resIndex-bundle" href="#" onclick="moldResults('$wuid-result-$resIndex', 'bundle'); return false;">download</a>]''')
...
def getResultLinkHtml(wuid, result, resultIndex, *args, **kargs):
    ...
    resLnkHtml = ''
    if outputName is not None: resLnkHtml = '%s: ' % outputName
    
    #loop over linkGenerators and harvest result links
    for lg in linkGenerators:
        if lg.recognized(result):
            resLnkHtml += lg.getLink(wuid, result, resultIndex, *args, **kargs) + ' '
