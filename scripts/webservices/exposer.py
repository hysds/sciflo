...
def main():
    ...
    for o, a in opts:
        ...
        if o in ("-w", "--wsdlDir"):
            if wsdlDirSet:
                print(
                    "Multiple -w|--wsdlDir specifications found.  Only specify one wsdl output directory.")
                usage()
                sys.exit(2)
            else:
                wsdlDir = a
        ...
        if o in ("-s", "--serveDir"):
            if serveDirSet:
                print(
                    "Multiple -s|--serveDir specifications found.  Only specify one root directory to serve.")
                usage()
                sys.exit(2)
            else:
                serveFiles = True
        ...
        if o in ("-e", "--executeDir"):
            if executeDirSet:
                print(
                    "Multiple -e|--executeDir specifications found.  Only specify one root directory to execute in.")
                usage()
                sys.exit(2)
            else:
                executeDir = a
    ...
    if not exposerProxyUrl:
        exposerProxyUrl = None
    ...
    for xmlDir in xmlDirs:
        xmlFiles.extend(glob.glob('%s/*.xml' % xmlDir))
    ...
    for xmlFile in xmlFiles:
        ...
        wsdlFile = None
        if wsdlDir:
            wsdlFile = os.path.join(wsdlDir, basename+'.wsdl')
